...
# Originally written for rasberrypi and allwinner sunxy boards.
# Adapt for the needs of beaglebone boards
#
...
IMAGE_TYPEDEP_bone-sdimg = "${SDIMG_ROOTFS_TYPE}"
...
# SD card image name
SDIMG = "${DEPLOY_DIR_IMAGE}/${IMAGE_NAME}.rootfs.bone-sdimg"

IMAGEDATESTAMP = "${@time.strftime('%Y.%m.%d',time.gmtime())}"

IMAGE_CMD_bone-sdimg () {
	...
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/${SPL_BINARY}-${MACHINE} ::${SPL_BINARY}
	mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/u-boot.${UBOOT_SUFFIX} ::u-boot.${UBOOT_SUFFIX}
